...
interface MatchData extends RowDataPacket {
  id: number;
...
function updateEloRatings(winnerElo: number, loserElo: number): { winner: number, loser: number } {
...
export async function calculateEloRankings(type: 'singles' | 'doubles'): Promise<EloRanking[]> {
  // Get all completed matches
  const [matches] = await pool.execute<MatchData[]>(`
  ...
  const playerRatings = new Map<number, PlayerRating>();

  // Process matches chronologically to update ratings
  filteredMatches.forEach(match => {
    // Get or create ratings for all players
    const team1Players: number[] = [match.team1_player1_id];
    if (match.team1_player2_id) team1Players.push(match.team1_player2_id);
    
    const team2Players: number[] = [];
    if (match.team2_player1_id) team2Players.push(match.team2_player1_id);
    if (match.team2_player2_id) team2Players.push(match.team2_player2_id);
  ...
  const rankings: EloRanking[] = Array.from(playerRatings.values())
