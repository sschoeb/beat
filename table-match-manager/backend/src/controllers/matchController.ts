...
export async function getCurrentMatch(_req: Request, res: Response): Promise<void> {
  try {
...
export async function startMatch(req: Request, res: Response): Promise<void> {
  try {
    ...
    const [playersInMatch] = await pool.execute<RowDataPacket[]>(`
...
export async function endMatch(req: Request, res: Response): Promise<void> {
  try {
    ...
    const [currentMatchRows] = await pool.execute<MatchRow[]>(`
      SELECT 
        m.id, m.winner_team, m.start_time, m.end_time, m.is_active,
        m.team1_player1_id, p1.name as team1_player1_name,
        m.team1_player2_id, p2.name as team1_player2_name,
    ...
    const [newMatchResult] = await pool.execute<ResultSetHeader>(`
    ...
    const [newMatchRows] = await pool.execute<MatchRow[]>(`
      SELECT 
        m.id, m.winner_team, m.start_time, m.end_time, m.is_active,
        m.team1_player1_id, p1.name as team1_player1_name,
        m.team1_player2_id, p2.name as team1_player2_name,
...
export async function cancelMatch(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (queueRows.length >= 2) {
      ...
      await pool.execute('DELETE FROM queue WHERE id IN (?, ?)', [team1.id, team2.id]);
      
      // Start new match
      const [nextMatchResult] = await pool.execute<ResultSetHeader>(`
        INSERT INTO matches (team1_player1_id, team1_player2_id, team2_player1_id, team2_player2_id) 
        VALUES (?, ?, ?, ?)
      `, [
      ...
      const [nextMatchRows] = await pool.execute<MatchRow[]>(`
        SELECT 
          m.id, m.winner_team, m.start_time, m.end_time, m.is_active,
          m.team1_player1_id, p1.name as team1_player1_name,
          m.team1_player2_id, p2.name as team1_player2_name,
      ...
      await pool.execute('DELETE FROM queue WHERE id = ?', [queueTeam.id]);
...
export async function forfeitMatch(req: Request, res: Response): Promise<void> {
  try {
...
export async function startMatchFromQueue(req: Request, res: Response): Promise<void> {
  try {
    ...
    // Remove from queue
    await pool.execute('DELETE FROM queue WHERE id = ?', [queueEntryId]);
