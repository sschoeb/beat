...
async function addToQueue(req, res) {
    try {
        ...
        const [activeMatchRows] = await connection_1.pool.execute(`
      SELECT 
    ...
    `);
        ...
        const [existingQueueRows] = await connection_1.pool.execute(`
...
async function getNextFromQueue(_req, res) {
    try {
        const [rows] = await connection_1.pool.execute(`
      SELECT 
        q.id, q.created_at,
    ...
    `);
        ...
        const queueEntry = mapRowToQueueEntry(rows[0]);
        await connection_1.pool.execute('DELETE FROM queue WHERE id = ?', [queueEntry.id]);
        res.json(queueEntry);
    }
    catch (error) {
...
async function removeFromQueue(req, res) {
    try {
        ...
        const [result] = await connection_1.pool.execute('DELETE FROM queue WHERE id = ?', [id]);
